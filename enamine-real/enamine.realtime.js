...
async function update(mrvSource) {
	...
	const results = await mlutils.post({
		url: server + "rest/descriptors/enamine-real-cfp7/find-most-similars",
		form: {
			query: mrvSource,
			"max-count": 20,
			metric: metric
...
module.exports = {
	...
	label: "Enamine Real",
	update: update,
	templateFile: "enamine.template.html",
	domains: ["*"],
	settings: [{
		label: "Search type",
		type: "enum",
	...
	sortOrder: 37,
	docs: "Searches the Enamine REAL database with using chemical hashed fingerprint. Similarity search uses tanimoto metric, substructure-like search sues Tversky metric with coefficients 0.0 and 1.0. The most similar 20 results will be displayed with no cutoff."
};